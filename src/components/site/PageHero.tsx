...
export function PageHero({ eyebrow, title, lede, image, mode = "car", seed = 0 }: { eyebrow: string; title: React.ReactNode; lede?: string; image?: string; mode?: "car" | "lifestyle"; seed?: number; }) {
  ...
  return (
    <section className="relative flex min-h-[78vh] items-end overflow-hidden border-b border-line">
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1400px] px-[6vw] pb-24 pt-40">
        ...
        <h1 className="max-w-[1100px] font-display uppercase leading-[1.02] text-ivory" style={{ fontSize: "clamp(42px,7vw,108px)", letterSpacing: "0.03em" }}>{title}</h1>
        {lede && <p className="mt-8 max-w-[560px] text-[16px] leading-[1.9] text-mute">{lede}</p>}
      </div>
    </section>
  );
}
